...
def PsnrC(img1, img2):
    ...
    mse = np.mean((img1 - img2) ** 2) # (SIGMA(x1-x2)^2)/n
    if mse < 1.0e-10:
        return 100
...
def main():
    ...
    parser.add_argument('--img-size', '-size',  nargs='+', default=[512, 512], type=int, help='The size of the images.')
    parser.add_argument('--model-pth', '-model', default=r'.\model_parameter\model_state.pth', type=str, help='The place of the model parameter.')
    parser.add_argument('--img-path', '-folder', default=r'.\standard_test_images', type=str, help='The place of the image to be predicted.')
    parser.add_argument('--mode', '-mode', default='histogram_shifting', type=str, help='The embedding ways.')
    parser.add_argument('--watermark-length', '-length', default=10000, type=int, help='The length of the watermark.')
    ...
    model = PredictModel(device) #model folder->predict_mpde.py file
    ...
    for i in range(0, len(img_file_list)):
        ...
        img_gray = np.array(img_gray, dtype=float)#Convert input image to gray image with size of 512*512
